    ...
    def read_serial(self):
        ...
        try:
            while self.serial.in_waiting >= 14:
                ...
                raw_x, raw_y, theta = struct.unpack("<fff", payload)

                # Convert MCU distance units to metres.  Angle is already in
                # radians, so only the planar position is scaled.
                x = raw_x * self.position_scale
                y = raw_y * self.position_scale

                # In dữ liệu linear (x, y) và theta nhận được
                self.get_logger().info(
                    f"linear: x={x:.3f} m, y={y:.3f} m | theta={theta:.3f} rad"
                )

                # BƠM DỮ LIỆU LÊN TOPIC /ODOM
